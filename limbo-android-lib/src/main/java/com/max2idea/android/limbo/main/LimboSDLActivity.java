...
public class LimboSDLActivity extends SDLActivity {
    ...
    private static int singleClickDelay = 100;
    private static int keyDelay = 75;
    private static int mouseButtonDelay = 100;
    ...
    final Object singleClickLock = new Object();

    public void singleClick(final MotionEvent event, final int pointer_id) {
        Thread t = new Thread(new Runnable() {
            public void run() {
                // Log.d("SDL", "Mouse Single Click");
                synchronized (singleClickLock) {
                    delay(singleClickDelay);
    ...
    public void sendCtrlAltKey(int code) {
        delay(keyDelay);
        sendNativeKey(null, KeyEvent.KEYCODE_CTRL_LEFT, true);
        delay(keyDelay);
        sendNativeKey(null, KeyEvent.KEYCODE_ALT_LEFT, true);
        delay(keyDelay);
        if (code >= 0) {
            sendNativeKey(null, code, true);
            delay(keyDelay);
            sendNativeKey(null, code, false);
            delay(keyDelay);
        }
        sendNativeKey(null, KeyEvent.KEYCODE_ALT_LEFT, false);
        delay(keyDelay);
        sendNativeKey(null, KeyEvent.KEYCODE_CTRL_LEFT, false);
    }
    ...
    public void checkStatus() {
        while (!timeQuit) {
            LimboActivity.VMStatus status = Machine.checkSaveVMStatus(this);
    ...
    public boolean onOptionsItemSelected(final MenuItem item) {
        ...
        if (item.getItemId() == R.id.itemDrives) {
            ...
            if (LimboActivity.currMachine.hasRemovableDevices()) {
                drives = new DrivesDialogBox(this, R.style.Transparent, this, LimboActivity.currMachine);
                drives.show();
            } else {
                UIUtils.toastShort(this, "No removable devices attached");
            }
        } else if (item.getItemId() == R.id.itemReset) {
            Machine.resetVM(this);
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                @Override
                public void run() {
                    toggleKeyboardFlag = UIUtils.onKeyboard(LimboSDLActivity.this, toggleKeyboardFlag, mSurface);
                }
            }, 200);
    ...
    private void onCtrlAltDel() {
        ...
        sendNativeKey(null, KeyEvent.KEYCODE_CTRL_RIGHT, true);
        sendNativeKey(null, KeyEvent.KEYCODE_ALT_RIGHT, true);
        sendNativeKey(null, KeyEvent.KEYCODE_FORWARD_DEL, true);
        sendNativeKey(null, KeyEvent.KEYCODE_FORWARD_DEL, false);
        sendNativeKey(null, KeyEvent.KEYCODE_ALT_RIGHT, false);
        sendNativeKey(null, KeyEvent.KEYCODE_CTRL_RIGHT, false);
    }

    private void onCtrlC() {

        sendNativeKey(null, KeyEvent.KEYCODE_CTRL_RIGHT, true);
        sendNativeKey(null, KeyEvent.KEYCODE_C, true);
        sendNativeKey(null, KeyEvent.KEYCODE_C, false);
        sendNativeKey(null, KeyEvent.KEYCODE_CTRL_RIGHT, false);
    }
    ...
    private void onStretchToScreen() {
        ...
        new Thread(new Runnable() {
            public void run() {
                ...
                if (Config.showToast)
                    UIUtils.toastShort(LimboSDLActivity.this, "Resizing, Please Wait");
                resize(null);
    ...
    public boolean setupMenu(Menu menu) {
        ...
        if (LimboSettingsManager.getAlwaysShowMenuToolbar(this) || Config.mouseMode == Config.MouseMode.External) {
        ...
        if (keyEvents != null)
            for (int i = 0; i < keyEvents.length; i++) {
                ...
                if (keyEvents[i].getAction() == KeyEvent.ACTION_DOWN) {
                    ...
                    sendNativeKey(null, keyEvents[i].getKeyCode(), true);
                } else if (keyEvents[i].getAction() == KeyEvent.ACTION_UP) {
                    // Log.v("sendText", "Down: " + keyEvents[i].getKeyCode());
                    sendNativeKey(null, keyEvents[i].getKeyCode(), false);
                }
            }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //XXX: if the app is terminated the last activity is in the recents
        // so we prevent crashing
        checkIfStarted();
    ...
    private void processMigrationResponse(String response) {
        ...
        if (errorStr != null) {
            ...
            new Handler(Looper.getMainLooper()).postDelayed(new Runnable() {
                @Override
                public void run() {
                    Machine.pausedErrorVM(LimboSDLActivity.this, descStr1);
                }
            }, 100);
...
//                vnc_canvas_layout_params.addRule(RelativeLayout.CENTER_IN_PARENT);
    ...
    public class LimboSDLSurface extends ExSDLSurface implements View.OnKeyListener, View.OnTouchListener {
        ...
        public LimboSDLSurface(Context context) {
            ...
            gestureDetector = new GestureDetector(LimboSDLActivity.this, new GestureListener());
        ...
        public synchronized void doResize(boolean reverse, final Configuration newConfig) {
            ...
            if (portrait) {
                if (Config.mouseMode != Config.MouseMode.External) {
                    int height_n = (int) (width / (vm_width / (float) vm_height));
        ...
        private boolean handleMissingKeys(int keyCode, int action) {
            ...
            if (action == KeyEvent.ACTION_DOWN) {
                sendNativeKey(null, 59, true);
                sendNativeKey(null, keyCodeTmp, true);
            } else {
                sendNativeKey(null, 59, false);
                sendNativeKey(null, keyCodeTmp, false);
            }
            return true;
        }
        ...
        public boolean onKey(View v, int keyCode, KeyEvent event) {
            ...
            if ((keyCode == KeyEvent.KEYCODE_BACK) || (keyCode == KeyEvent.KEYCODE_FORWARD)) {
                ...
                sendKeyAsync(event, keyCode, event.getAction(), true);
                return true;
            } else if (event.getAction() == KeyEvent.ACTION_UP) {
                //Log.v("SDL", "key up: " + keyCode);
                sendKeyAsync(event, keyCode, event.getAction(), false);
                return true;
            } else {
        ...
        final Object sendKeyLock = new Object();

        private void sendKeyAsync(final KeyEvent event, final int keyCode, final int action,
                                  final boolean down) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    synchronized (sendKeyLock) {
                        long now = System.currentTimeMillis();
                        if (!handleMissingKeys(keyCode, action)) {
                            if (down) {
                                sendNativeKey(event, keyCode, true);
                            } else {
                                sendNativeKey(event, keyCode, false);
                            }
                        }
                    }
                }
        ...
        public boolean onTouchProcess(View v, MotionEvent event) {
            sendMouseActionAsync(v, event, event.getAction());
            return true;
        }

        final Object sendMouseButtonLock = new Object();
        final Object sendMouseMoveLock = new Object();

        public void sendMouseActionAsync(View v, final MotionEvent event, final int action) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    ...
                    if (action == MotionEvent.ACTION_MOVE) {
                        synchronized (sendMouseMoveLock) {
                            if (mouseUp) {
                                old_x = x;
                                old_y = y;
                                mouseUp = false;
                            ...
                            if (action == MotionEvent.ACTION_MOVE) {
                                if (Config.mouseMode == Config.MouseMode.External) {
                                    //Log.d("SDL", "onTouch absolute delta =" + action + ", X,Y=" + (x) + "," + (y) + " P=" + p);
                                    LimboActivity.vmexecutor.onLimboMouse(0, MotionEvent.ACTION_MOVE, 0, x, y);
                                } else {
                                    //Log.d("SDL", "onTouch relative delta =" + action + ", X,Y=" + (x-old_x) + "," + (y - old_y) + " P=" + p);
        ...
        private void processMouseButton(MotionEvent event, int action, float x, float y) {
            synchronized (sendMouseButtonLock) {
                ...
                } else if (event.getAction() == MotionEvent.ACTION_DOWN && Config.mouseMode == Config.MouseMode.External) {
...
//                            Log.d("SDL", "Mouse Scroll: " + x + "," + y);
    ...
    final static Object sendNativeKeyLock = new Object();
    private static void sendNativeKey(KeyEvent event, int keycode, boolean down) {
        synchronized (sendNativeKeyLock) {
            if (down)
